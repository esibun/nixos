...
let
  ...
  '';
  script = writeShellScriptBin shortname (baseScript ''${scope} ${commandPrefix} ${pkgs.gamemode}/bin/gamemoderun ${if useGamescope then "${pkgs.gamescope}/bin/gamescope ${config.gamescopeFlags} ${extraGamescopeFlags} -- " else ""}${gamePrefix} ${exeCommand} "${gameDir}${mainBinary}" ${gamePostfix} ${commandPostfix}'');
